...
public class RobotContainer {
  ...
  public Command drivePath(boolean isFirstPath, String nameOfPath) {
    ...
    PathPlannerTrajectory drivePath1 = PathPlanner.loadPath(nameOfPath, new PathConstraints(0.4, 0.2));
    ...
    return new SequentialCommandGroup(
      new InstantCommand(() -> {
        ...
        if(isFirstPath){
            driveTrain.resetOdometry(drivePath1.getInitialPose());
        }
      }),
      new PPRamseteCommand(
          ...
          new PIDController(0.5, 0.1, 0.3), // Left controller. Tune these values for your robot. Leaving them 0 will only use feedforwards.
          new PIDController(0.5, 0.1, 0.3), // Right controller (usually the same values as left controller)
...
public Command getAutonomousCommand(boolean isFirstPath){
   return new SequentialCommandGroup(drivePath(true, "Pos2Ez"), visionAlign());
